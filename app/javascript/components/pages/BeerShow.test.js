...
import beers from "../../MockBeer";
    ...
    const loggedIn =() => {
      render(
        <MemoryRouter initialEntries={["/beershow/1"]}>
            <Routes>
                <Route path="/beershow/:id" element={<BeerShow beers={beers} props={{logged_in:true}} />} />
          </Routes>
        </MemoryRouter>
      );
    }
    const loggedInMyBeer = () => {
      render(
        <MemoryRouter initialEntries={["/beershow/1"]}>
          <Routes>
            <Route path="/beershow/:id" element={<BeerShow beers={beers} props={{logged_in:true, current_user:{id:1}}} />} />
          </Routes>
        </MemoryRouter>
      )
    }
